...
for (const [label, source] of [
  ...
  ["StickyEditorActions", stickyEditorActions],
  ["ExerciseEditor", exerciseEditor],
]) {
  assert.ok(source.includes("icon-button--save"), `${label} muss die gemeinsame Save-Variante verwenden.`);
}

for (const legacyClass of ["editor-shell-save", "editor-save-button", "exercise-editor-save-button"]) {
  assert.equal(
    [editorShell, stickyEditorActions, exerciseEditor].some((source) => source.includes(legacyClass)),
    false,
...
console.log(`UI-Control-Standard erfolgreich: 42/38/40-Geometrie, semantische Iconaktionen, Tabs, Auswahlzeilen, Switches und ${labeledFieldCount} integrierte Formularfelder sind zentral abgesichert.`);
