...
test("Auswahllisten bearbeiten unterhalb der globalen Kopfzeile", async ({ page }) => {
  ...
  const saveButton = page.getByTestId("dropdown-setting-save");
  ...
  const dropdownField = editor.locator(".ui-labeled-field").first();
  await expect(dropdownField).toBeVisible();
  await expect(dropdownField).toHaveCSS("border-radius", "10px");
  await expect(dropdownField.locator(".ui-field-label")).toHaveCSS("background-color", "rgb(241, 244, 242)");
  await expect(dropdownField.locator(".ui-field-control")).toHaveCSS("border-top-width", "0px");
...
test("D2 Übungskatalog zeigt kompakte Liste, Schnellinfos und Filter-Sheet", { tag: "@pr" }, async ({ page }) => {
  ...
  const exerciseNameField = page.locator(".exercise-editor-page .ui-labeled-field").first();
  await expect(exerciseNameField).toBeVisible();
  await expect(exerciseNameField).toHaveCSS("border-radius", "10px");
  await expect(exerciseNameField.locator(".ui-field-label")).toHaveCSS("background-color", "rgb(241, 244, 242)");
  await expect(exerciseNameField.locator(".ui-field-control")).toHaveCSS("border-top-width", "0px");
  const appHeaderBox = await page.locator(".app-header").boundingBox();
