...
import { CalendarClock, CalendarDays, Info, Layers3 } from "lucide-react";
...
type GroupSection = "master" | "training" | "performance";

const GROUP_SECTIONS = ["master", "training", "performance"] as const;
const GROUP_FORM_ID = "training-group-editor-form";
...
export function TrainingGroupEditor({
  ...
  const [section, setSection] = useState<GroupSection>("master");
  const [error, setError] = useState<string | null>(null);
  useDraftDirtyState(values, onDirtyChange);

  const canSave = values.name.trim().length >= 2
    ...
    <section className="management-editor training-group-editor compact-editor" aria-label={mode.type === "create" ? "Gruppe anlegen" : "Gruppe bearbeiten"}>
      ...
      >
        <div className="editor-section-tabs" role="tablist" aria-label="Gruppenbereiche">
          ...
          <button type="button" role="tab" aria-selected={section === "training"} className={section === "training" ? "active" : ""} onClick={() => setSection("training")}>
            <CalendarDays aria-hidden="true" /> Training <span>{values.regularWeekdays.length}</span>
          </button>
          <button type="button" role="tab" aria-selected={section === "performance"} className={section === "performance" ? "active" : ""} onClick={() => setSection("performance")}>
            <Layers3 aria-hidden="true" /> Leistung
          </button>
        </div>
        ...
        <fieldset className="athlete-editor-lock-fieldset" disabled={!canEdit || busy}>
          ...
          {section === "training" && (
            <div className="editor-section-panel group-training-settings" role="tabpanel">
              <fieldset className="group-schedule-fieldset editor-section-card">
                <legend><CalendarDays aria-hidden="true" /> Regelmäßige Trainingstage</legend>
                <p className="field-hint">Diese Wochentage steuern die auswählbaren Trainingstermine.</p>
                <div className="weekday-selector" aria-label="Trainingstage auswählen">
                  {WEEKDAYS.map((weekday) => {
                    ...
                    return (
                      <button type="button" className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => toggleWeekday(weekday.value)} key={weekday.value}>
                        {weekday.label}
                      </button>
                    );
                  })}
              ...
              <div className="group-module-settings editor-section-card">
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => setValues((current) => ({
                      ...current,
                      moduleKey: event.target.value === "kindertraining" || event.target.value === "u12" || event.target.value === "u14"
                        ? event.target.value
                        : null,
                    }))}
                  >
                ...
                <label className="setting-checkbox">
                  <input
                    ...
                    onChange={(event) => setValues((current) => ({ ...current, allowSpecialTraining: event.target.checked }))}
                  />
                  <span>
              ...
              {values.moduleKey !== null && values.regularWeekdays.length === 0 && (
                <div className="alert warning compact-alert">Für ein Trainingsmodul muss mindestens ein Wochentag ausgewählt sein.</div>
              )}
            </div>
          )}

          {section === "performance" && (
            <fieldset className="performance-group-fieldset performance-group-fieldset-v2 editor-section-panel editor-section-card" role="tabpanel">
              <legend><CalendarClock aria-hidden="true" /> Leistungsgruppe</legend>
              ...
              {values.isPerformanceGroup ? (
                  ...
                  <div className="performance-setting-row deadline-row">
                    ...
                    <div className="performance-deadline-inputs">
                      <select
                        ...
                        onChange={(event) => setValues((current) => ({ ...current, registrationDeadlineWeekday: Number(event.target.value) }))}
                      >
                        {WEEKDAYS.map((weekday) => <option value={weekday.value} key={weekday.value}>{weekday.label}</option>)}
                      </select>
                      <input
                        type="time"
                        aria-label="Uhrzeit des Anmeldeschlusses"
                        value={values.registrationDeadlineTime}
                        onChange={(event) => setValues((current) => ({ ...current, registrationDeadlineTime: event.target.value || "18:00" }))}
                      />
                    </div>
                  </div>
                ...
                <div className="inline-empty-state compact-empty-state">Diese Gruppe verwendet keine wochenweise Trainingsanmeldung.</div>
              )}

              {values.isPerformanceGroup && values.regularWeekdays.length === 0 && (
                <div className="alert warning compact-alert">Für eine Leistungsgruppe muss im Reiter „Training“ mindestens ein Wochentag ausgewählt sein.</div>
              )}
            </fieldset>
          )}
        </fieldset>
